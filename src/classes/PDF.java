...
public class PDF {
    ...
    public void generateMasterPlanPDFUpdate(JTable table, String[] data){
         try {
            Document doc = new Document(PageSize.A4.rotate(), 20, 20, 20, 20);
            ...
            PdfWriter.getInstance(doc, new FileOutputStream(tempPath + "RM_Order_Master_Plan-" + data[0] + "(Updated).pdf"));
            doc.open();
            ...
            //Adding data from master table
            String category = table.getValueAt(0, 6).toString();
            masterTable.addCell(getIngCategoryCell(category.toUpperCase()));
            
            for(int i=0; i<table.getRowCount(); i++){
                if (!category.equals(table.getValueAt(i, 6).toString())){
                    category = table.getValueAt(i, 6).toString();
                    masterTable.addCell(getIngCategoryCell(category.toUpperCase()));
                }
                masterTable.addCell(getTableDataCell(table.getValueAt(i, 0).toString()));
                masterTable.addCell(getTableDataCell(table.getValueAt(i, 1).toString()));
